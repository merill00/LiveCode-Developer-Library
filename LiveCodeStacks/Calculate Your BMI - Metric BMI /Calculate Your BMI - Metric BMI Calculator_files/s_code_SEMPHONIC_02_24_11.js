...
try{
	if(omn_account){
		s_account=omn_account;
	}
	else{
		s_account="nhlbiglobal";
	}
}
catch(err){
	s_account="nhlbiglobal";
...
s.linkDownloadFileTypes="exe,zip,wav,wmv,mp3,mp4,m4a,mov,mpg,txt,rm,avi,wmv,doc,docx,pdf,xls,xlsx,csv,ppt,pptx" //modified by SG 04/22/2013 //"exe,zip,wav,mp3,mp4,mov,mpg,avi,wmv,doc,docx,,pdf,xls,xlsx"
s.linkInternalFilters="javascript:,nhlbi.nih.gov,nhlbihin.net,nhlbisupport.com"  //modify by FQ 8.2011
...
//pageName logic
//s.pageName = document.location.hostname + document.location.pathname;
//s.pageName = document.title;

<!-- Scope: set File Path for variable pageName  -->

if(document.location.hostname.indexOf("healthyeating.nhlbi.nih.gov") >= 0)
{
    s.pageName = 'healthyeating' + document.location.pathname;

}
else if(document.location.hostname.indexOf("cvdrisk") >= 0)
{
    s.pageName = 'cvdrisk' + document.location.pathname;
}
else
{
s.pageName = 'nhlbi' + document.location.pathname;
varPageName = s.pageName;
}
<!-- End -->

//channel set to value defined in channel_code.js
s.channel = omn_channel;

//Capture full URL in two props
...
//Capture full URL for tracking campaigns
var fullTrackingURL = window.location.href;
s.prop46 = fullTrackingURL;


//Capture document title in a prop
...
//previous Page
s.prop34=s.getPreviousValue(s.pageName,'gpv_pn',"");
//s.prop34=s.getPreviousValue(s.pageName,'gpv_pn','event2');

//custom page view event
s.events=s.apl(s.events,"event1",",",1);


//Capture campaign codes and apply the getValOnce plugin
//adjusting cid to utm for heart truth analytics campaign 2/3/2017
//s.campaign=s.getQueryParam('utm');
//s.campaign=s.getValOnce(s.campaign,"s_camp",0);

s.campaign=s.getQueryParam('utm_source,utm_campaign,utm_medium,utm_term,utm_content',':');




//Copy pageName to an eVar
s.eVar1 = s.pageName;
...
//Audience
s.prop8 = getMetaValue('Audience');
s.eVar8 = s.prop8;

//Set language to english if Langage meta tag is not present
var omn_language = getMetaValue('Language');
if(omn_language == null || typeof(omn_language) == undefined){
	s.prop9 = "english";
	}
else{
	s.prop9 = omn_language;
	}


//Date content last modified
s.prop17 = getMetaValue('Last.Revised');
s.eVar17 = s.prop17;
...
s.visitorNamespace="nhlbi"
s.trackingServer="nhlbi.122.2o7.net"
